 ...
 * @version 1.3.2 (Fix: Unused Variable)
 * @brief AST implementation. Cleaned up warnings.
 */
...
Token Tokenizer::next() {
    ...
    if (ch == '"') {
        // FIX: Removed unused 'start' variable
        ++pos;
        std::string val;
        while (pos < input.size() && input[pos] != '"') val += input[pos++];
        if (pos >= input.size()) throw std::runtime_error("Unterminated string literal");
        ++pos; return {TokenType::STRING, val};
    }

    if (isdigit(ch) || (ch == '-' && pos + 1 < input.size() && isdigit(input[pos + 1]))) {
        size_t start = pos; if (input[pos] == '-') ++pos;
    ...
    if (ch == '=' || ch == '!' || ch == '<' || ch == '>') {
        std::string op(1, ch); ++pos;
        if (pos < input.size() && input[pos] == '=') op += input[pos++];
...
int StringCompareNode::eval() const {
    if (!m_bound_string_func) return 0;
...
ASTPtr FnParser::parsePrimary() {
    if (current.type == TokenType::NUMBER) {
        int val = std::stoi(current.value); advance(); return std::make_unique<ConstNode>(val);
    }
    if (current.type == TokenType::IDENT) {
        std::string name = current.value; advance();
        if (current.type == TokenType::LPAREN) {
            advance(); std::vector<ASTPtr> args;
            if (current.type != TokenType::RPAREN) {
                do { args.push_back(parseExpression()); if (current.type == TokenType::COMMA) advance(); } while (current.type != TokenType::RPAREN);
            }
            advance(); return std::make_unique<FuncCallNode>(name, std::move(args));
        }
        throw std::runtime_error("Expected function call");
    }
    if (current.type == TokenType::OP && current.value == "!") {
        std::string op = current.value; advance(); return std::make_unique<UnaryNode>(op, parsePrimary());
    }
    if (current.type == TokenType::LPAREN) {
        advance(); ASTPtr expr = parseExpression();
...
ASTPtr FnParser::parseComparison() {
    ...
    while (current.type == TokenType::OP && (current.value == "==" || current.value == "!=" || current.value == "<" || current.value == ">" || current.value == "<=" || current.value == ">=")) {
        std::string op = current.value; advance();
        if (current.type == TokenType::STRING) {
            std::string targetStr = current.value; advance();
            auto* funcNode = dynamic_cast<FuncCallNode*>(left.get());
            if (!funcNode) throw std::runtime_error("String comparison requires function on left.");
            auto strNode = std::make_unique<StringCompareNode>(funcNode->name, std::move(funcNode->args), op, targetStr);
            left = std::move(strNode);
        } else {
            ASTPtr right = parsePrimary(); left = std::make_unique<BinaryNode>(std::move(left), op, std::move(right));
        }
    }
    return left;
}
ASTPtr FnParser::parseAnd() {
    ...
    while (current.type == TokenType::OP && current.value == "AND") { std::string op = current.value; advance(); ASTPtr right = parseComparison(); left = std::make_unique<BinaryNode>(std::move(left), op, std::move(right)); }
    return left;
}
ASTPtr FnParser::parseOr() {
    ASTPtr left = parseAnd();
    while (current.type == TokenType::OP && current.value == "OR") { std::string op = current.value; advance(); ASTPtr right = parseAnd(); left = std::make_unique<BinaryNode>(std::move(left), op, std::move(right)); }
    return left;
}
